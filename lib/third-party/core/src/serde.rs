pub mod deserialize_rfc3339_epoch_second {
    ...
    //!
    //! This module transparently handles both formats to deserialize to [`DateTime<Utc>`](chrono::DateTime). It mostly
    ...
    {
        deserializer.deserialize_any(Rfc3339EpochSecondTimestampVisitor)
    }
    ...
    where
        D: de::Deserializer<'de>,
    {
        Option::<WrappedRfc3339EpochSecondTimestamp>::deserialize(deserializer)
            .map(|opt_wrapped| opt_wrapped.map(|wrapped| wrapped.0))
    }

    struct Rfc3339EpochSecondTimestampVisitor;

    impl de::Visitor<'_> for Rfc3339EpochSecondTimestampVisitor {
